...
KEGG_DIAMONDOUT_DIR = PROJECT_DIR + "/" + config["KEGG_DIAMOND_OUT_DIR"]
...
rule all:
    input:
        expand(KEGG_DIAMONDOUT_DIR + "/{sample}_against_kegg.diamondout", sample = SAMPLE_IDS)
...
rule diamond_kegg:
    ...
    params:
        ...
        KEGG_diamond_DB = config["KEGG_diamond_DB"]
    output:
        KEGG_DIAMONDOUT_DIR + "/{sample}_against_kegg.diamondout"
    shell:
        """
        mkdir -p {params.OUTPUT_DIR}
        diamond blastx \
        -d {params.KEGG_diamond_DB} \
        -q {input.R1_FASTA} \
        -o {output}
        """
